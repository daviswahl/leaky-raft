...
fn start_server<P: AsRef<str> + fmt::Display>(port: P, ports: P) -> Child {
    let file = OpenOptions::new()
        ...
        .create(true)
        .truncate(true)
        .open(log_path(port.as_ref()))
        .unwrap();
    let mut cmd = Command::new("target/debug/main");

    let pid = format!("logs/{}.pid", port);
...
fn main() {
    use std::sync;
    let children = sync::Arc::new(sync::Mutex::new(vec![]));
    ...
    children
        .lock()
        .unwrap()
        .push(start_server("13000", "13001,13002"));
    children
        .lock()
        .unwrap()
        .push(start_server("13001", "13002,13000"));
    children
        .lock()
        .unwrap()
        .push(start_server("13002", "13000,13001"));

    let children_killer = children.clone();
